...
from flask import Flask, request, render_template, stream_template, Blueprint, current_app
...
def create_app(config_filename: str="config.toml") -> Flask:
    ...
    app.config.from_file(os.path.join(os.getcwd(), config_filename), load=tomllib.load, text=False)
    ...
    if app.config.get('FETCHER_COBALT_ENABLED') is True:
        ...
        fetchers.append(CobaltFetcher(app.config.get('FETCHER_COBALT_URL', 'http://localhost:9000'),
